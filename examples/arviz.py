...
def fill_test_layer(layer: viz.Layer, time_sec):
    ...
    layer.elements += [
        viz.Box(
            "box", position=(100 * np.sin(time_sec), 0, 0), size=100, color=(255, 0, 0)
        ),
        viz.Ellipsoid(
            "ellipsoid",
            position=(0, 100, 150),
        ...
        viz.Cylinder(
            "cylinder",
            position=(150, 100 * np.sin(time_sec), 0),
            radius=50,
            height=100,
            color=(0, 255, 0),
        ),
        ...
        viz.Text(
            "text", text="Test Text", position=(0, 0, 300), scale=4.0, color=(0, 0, 0)
        ),
    ]
...
def fill_example_layer(layer: viz.Layer, time_sec):
    ...
    poly = viz.Polygon(
        "poly", position=(1500, 0, 0), color=(255, 128, 0, 128), line_width=0.0
    )
    ...
    mesh.faces = [[0, 1, 2, 0, 1, 2], [1, 2, 3, 0, 1, 2]]
    layer.add(mesh)
...
def fill_points_layer(layer: viz.Layer, time_sec):
    pc = viz.PointCloud(
        "points", position=(2000, 0, 400), transparency=0.0, point_size=3
    )
    num = 50
...
def fill_objects_layer(layer: viz.Layer, time_sec):
    layer.elements += [
        viz.Object(
            "Amicelli",
            position=(100 * np.sin(time_sec), 1000, 0),
        ...
        viz.Object(
            "workbench",
            position=(300 + 100 * np.sin(time_sec), 1000, 0),
            project="ArmarXObjects",
...
def main():
    ...
    permanent_layer.elements += [
        viz.Box("box", position=(2000, 0, 0), size=200, color=(255, 165, 0))
    ]
