...
def run_bimanual_imp_ctrl(
    ...
    try:
        ...
        for i in range(n_steps):
            ...
            target_pose = {
                Armar6NodeSet.LeftArm.value: target_left.tolist(),
                Armar6NodeSet.RightArm.value: target_right.tolist()
            }
            target_nullspace = {
                Armar6NodeSet.LeftArm.value: [],
                Armar6NodeSet.RightArm.value: []
            }
            # target_nullspace = Ice.Unset
            start_ice = time.time()
            robot.set_control_target(controller_name, target_pose, target_nullspace)
            time_ice_list.append(time.time() - start_ice)
    ...
    console.log(f"frequency: {len(time_list)/control_end}")
    plt.plot(np.linspace(0, control_end, len(time_list)), time_list, "b-")
    plt.plot(np.linspace(0, control_end, len(time_list)), time_ice_list, "r-")
    plt.show()
    console.rule("done")
